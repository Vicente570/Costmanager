...
import { Wifi, WifiOff, RefreshCw, Clock, TrendingUp, AlertCircle, Settings, ChevronDown, ChevronUp, Calendar, Timer, CheckCircle } from 'lucide-react';
...
export function RealTimeIndicator({ 
  ...
  const [lastConnectionState, setLastConnectionState] = useState(connectionError);

  useEffect(() => {
    // Si la conexión se restablece (de error a conectado)
    if (lastConnectionState && !connectionError) {
      setShowReconnected(true);
      setTimeout(() => setShowReconnected(false), 3000);
    }
    setLastConnectionState(connectionError);
  }, [connectionError, lastConnectionState]);
      ...
      <div className="fixed bottom-4 right-4 z-50">
        <div className="bg-red-100 border border-red-300 rounded-lg p-3 shadow-lg">
          ...
          <p className="text-xs text-red-600 mt-1">
            {language === 'es' ? 'Intentando reconectar...' : 'Attempting to reconnect...'}
          </p>
        </div>
      </div>
    );
  ...
  if (showReconnected) {
    return (
      <div className="fixed bottom-4 right-4 z-50">
        <div className="bg-green-100 border border-green-300 rounded-lg p-3 shadow-lg animate-in slide-in-from-right">
          <div className="flex items-center space-x-2">
            <CheckCircle className="w-4 h-4 text-green-600" />
            <span className="text-sm text-green-700 font-medium">
              {language === 'es' ? 'Reconectado' : 'Reconnected'}
            </span>
          </div>
          <p className="text-xs text-green-600 mt-1">
            {language === 'es' ? 'Conexión restablecida' : 'Connection restored'}
          </p>
        </div>
      </div>
    );
